...
export const M41B_REQUIRED_QA_STEPS = [
  "migration_integrity",
  "focused_m41b_tests",
  "typecheck",
  "strict_lint",
...
export function parseM41bQaOptions(argv) {
  let root;
  let output;
  ...
  return { root: resolvedRoot, output: path.resolve(output), migrationOnly };
}
...
export function discoverFocusedM41bTests(sourceRoot) {
  const candidates = [];
  for (const directoryName of ["api/tests", "src"]) {
    ...
    for (const relativePath of walkFiles(directory)) {
      ...
      if (/^m41b-.+\.test\.tsx?$/.test(fileName))
...
function writeMigrationLog(logDirectory, record) {
  atomicWrite(
    path.join(logDirectory, "migration_integrity.log"),
    [
...
function writeMigrationReport(output, migrationDatabase, record) {
  assert(
    ...
    "M4.1B migration verification did not produce a passing transient database.",
  );
  let result;
  try {
    ...
    throw new Error(
      `M4.1B migration verification did not emit valid JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  assert(
    result?.milestone === "M4.1B" &&
      ...
      result.passed === true,
    "M4.1B migration verification result is not passing synthetic evidence.",
  );
  const databaseHash = hashFile(migrationDatabase);
  const report = {
    schemaVersion: "1.0",
    reportId: "M41B-MIGRATION-VERIFICATION",
    milestone: "M4.1B",
  ...
  atomicWrite(
    path.join(output, "M4_1B_MIGRATION_VERIFICATION.json"),
...
function executeMigrationVerification(sourceRoot, root, output) {
  assert(
    ...
    "M4.1B migration evidence cannot be written inside the canonical source tree.",
  );
  const logDirectory = path.join(output, "Verification_Logs");
  fs.mkdirSync(logDirectory, { recursive: true });
  const migrationDatabase = path.join(
    os.tmpdir(),
    `amos-m41b-migration-${process.pid}-${Date.now()}.db`,
  );
  const command = [
    process.execPath,
    "scripts/m41b-verify-migrations.mjs",
    "--root",
    root,
    "--output",
    migrationDatabase,
  ...
  try {
    const execution = spawnSync(command[0], command.slice(1), {
      ...
      env: {
        ...
        npm_config_cache:
          ...
          path.join(os.tmpdir(), "npm-cache-amos-m41b"),
      },
    });
    const record = {
    ...
    writeMigrationLog(logDirectory, record);
    if (record.exitCode !== 0)
      throw new Error(
        `M4.1B migration verification failed (${record.exitCode}).`,
      );
...
export function runM41bMigrationVerification(options) {
...
export function runM41bQa(options) {
  const sourceRoot = sourceRootFor(options.root);
  assert(
    !isWithin(sourceRoot, options.output),
    "M4.1B QA output cannot be written inside the canonical source tree.",
  );
  const focusedTests = discoverFocusedM41bTests(sourceRoot);
  assert(focusedTests.length > 0, "No focused M4.1B tests were discovered.");
  const logDirectory = path.join(options.output, "Verification_Logs");
  fs.mkdirSync(logDirectory, { recursive: true });
  const migrationDatabase = path.join(
    os.tmpdir(),
    `amos-m41b-qa-${process.pid}-${Date.now()}.db`,
  );
  const steps = [
    {
      id: "migration_integrity",
    ...
    { id: "server_build", command: "npm", args: ["run", "build:server"] },
    {
      ...
      args: [
        "--import",
        "tsx",
        "scripts/m41b-export-evidence.ts",
        "--root",
        options.root,
        "--output",
        options.output,
    ...
    },
    {
      ...
      args: [
        "--import",
        "tsx",
        "scripts/m41b-verify-evidence.ts",
        "--root",
        options.root,
        "--output",
        options.output,
  ...
  const report = {
    ...
    completedAt: new Date().toISOString(),
    focusedTests,
    requiredSteps: M41B_REQUIRED_QA_STEPS,
    migrationLog: "Verification_Logs/migration_integrity.log",
    migrationReport: "M4_1B_MIGRATION_VERIFICATION.json",
    passed:
  ...
  atomicWrite(
    path.join(options.output, "M4_1B_INTEGRATED_QA.json"),
    `${JSON.stringify(report, null, 2)}\n`,
  );
  return report;
}
...
if (isMain) {
  try {
    const options = parseM41bQaOptions(process.argv.slice(2));
    const report = options.migrationOnly
      ? runM41bMigrationVerification(options)
      : runM41bQa(options);
