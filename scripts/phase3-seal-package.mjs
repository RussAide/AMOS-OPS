...
const PACKAGE_BASE_NAME = "AMOS-OPS_Phase_3_Integrated_Sprint_Package_v1_0";
const SOURCE_SNAPSHOT_NAME = "AMOS-OPS_Phase_3_Canonical_Source_Snapshot_v1_0.zip";
const MILESTONE_DIRECTORIES = [
  "M3.1_Compliance_and_Audit_System_Operational",
  "M3.2_Revenue_Cycle_Functional",
  "M3.3_Workforce_Management_Live",
  "M3.4_GAD_Operations_Complete",
];
...
export function parsePhase3SealOptions(argv) {
  let root;
  let output;
  const positional = [];
...
function verifyQaReport(sharedRoot) {
  const reportPath = path.join(sharedRoot, "PHASE_3_INTEGRATED_QA.json");
  assert(fs.existsSync(reportPath), `Phase 3 integrated QA report is missing: ${reportPath}`);
  const report = JSON.parse(fs.readFileSync(reportPath, "utf8"));
  assert(report.reportId === "PHASE3-INTEGRATED-QA" && report.evidenceClass === "synthetic_demo", "Phase 3 integrated QA identity or evidence boundary drifted.");
  assert(report.passed === true, "Phase 3 integrated QA is not passing.");
  assert(JSON.stringify(report.requiredSteps) === JSON.stringify(REQUIRED_QA_STEPS), "Phase 3 required QA step register drifted.");
  assert(Array.isArray(report.focusedTests) && report.focusedTests.length > 0, "Phase 3 QA did not retain focused test identities.");
  assert(Array.isArray(report.steps) && JSON.stringify(report.steps.map((step) => step.id)) === JSON.stringify(REQUIRED_QA_STEPS), "Phase 3 executed QA step order drifted.");
  assert(report.steps.every((step) => step.exitCode === 0), "Phase 3 integrated QA contains a failing step.");
  for (const step of report.steps) {
    assert(typeof step.log === "string" && !path.isAbsolute(step.log), `Phase 3 QA log path is invalid for ${step.id}.`);
    const logPath = path.resolve(sharedRoot, step.log);
    assert(isWithin(sharedRoot, logPath) && fs.existsSync(logPath), `Phase 3 QA log is missing for ${step.id}.`);
  }
  return report;
}

export function sealPhase3Package(options) {
  ...
  const sharedRoot = path.join(evidenceRoot, "shared");
  for (const [label, target] of [["source", sourceRoot], ["controls", controlsRoot], ["evidence", evidenceRoot], ["shared evidence", sharedRoot]]) {
    assert(fs.existsSync(target) && fs.statSync(target).isDirectory(), `Phase 3 ${label} root is missing: ${target}`);
  }
  assert(!isWithin(sourceRoot, options.output), "Phase 3 package output cannot be written inside the canonical source tree.");

  run(process.execPath, [
    "--import",
    "tsx",
    path.join(SCRIPT_DIRECTORY, "phase3-verify-evidence.ts"),
    "--root",
    phaseRoot,
    "--output",
    sharedRoot,
  ...
  const acceptanceManifest = JSON.parse(fs.readFileSync(path.join(sharedRoot, "PHASE_3_ACCEPTANCE_MANIFEST.json"), "utf8"));
  assert(acceptanceManifest.status === "complete" && acceptanceManifest.evidenceClass === "synthetic_demo", "Final sealing requires complete synthetic Phase 3 acceptance evidence.");
  assert(acceptanceManifest.criteriaExpected === 31 && acceptanceManifest.criteriaPassed === 31, "Final sealing requires all 31 Phase 3 criteria.");
  assert(acceptanceManifest.exitGate === true, "Final sealing requires the passing Phase 3 exit gate.");
  const qaReport = verifyQaReport(sharedRoot);
  ...
  for (const target of [packageZip, packageSidecar, sealResultPath]) assert(!fs.existsSync(target), `Phase 3 sealing refuses to overwrite existing output: ${target}`);

  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "amos-phase3-seal-"));
  try {
    ...
    assert(stagedSourceMap.size > 0 && stagedSourceMap.has("package.json"), "Filtered Phase 3 canonical source snapshot is incomplete.");
    assert([...stagedSourceMap.keys()].every((relativePath) => !sourcePathExcluded(relativePath)), "Filtered Phase 3 source still contains excluded content.");
    ...
    for (const directoryName of [...MILESTONE_DIRECTORIES, "shared"]) {
      ...
      assert(fs.existsSync(source) && fs.statSync(source).isDirectory(), `Phase 3 package evidence folder is missing: ${directoryName}`);
      fs.cpSync(source, path.join(packageEvidence, directoryName), { recursive: true, preserveTimestamps: true });
    }
    ...
    assert(!evidenceFiles.some((relativePath) => relativePath.split("/").some((segment) => ["source", "node_modules", ".git", "dist", "dist-server", "coverage"].includes(segment))), "Phase 3 evidence contains a prohibited source, dependency, or build-output copy.");
    assert(!evidenceFiles.some((relativePath) => /\.(zip|db|sqlite|sqlite3)(-shm|-wal)?$/i.test(relativePath)), "Phase 3 evidence contains a nested snapshot or runtime database.");
    const sourceEntries = walkFiles(packageSource);
    assert(sourceEntries.length === 1 && sourceEntries[0] === SOURCE_SNAPSHOT_NAME, "Phase 3 package must contain exactly one canonical source snapshot and no loose source copy.");

    fs.writeFileSync(path.join(packageRoot, "README.md"), `# AMOS-OPS Phase 3 Integrated Sprint Package

This package contains one controlling-document set, one integrated Phase 3 evidence set, and one filtered canonical source snapshot for M3.1, M3.2, M3.3, and M3.4.
...
`);
    ...
    const packageManifestPath = path.join(packageManifestRoot, "PHASE_3_PACKAGE_MANIFEST.json");
    ...
    fs.writeFileSync(
      path.join(packageManifestRoot, "PHASE_3_PACKAGE_SHA256SUMS.txt"),
    ...
    compareMaps(stagedPackageMap, fileMap(path.join(packageRestore, PACKAGE_BASE_NAME)), "Integrated Phase 3 package restoration");
    ...
    assert(sourceSnapshots.length === 1 && sourceSnapshots[0].endsWith(`/${SOURCE_SNAPSHOT_NAME}`), `Sealed Phase 3 package contains ${sourceSnapshots.length} canonical source snapshots; exactly one is required.`);
    assert(outerListing.filter((entry) => entry.endsWith("/PHASE_3_INTEGRATED_SCENARIO_RESULT.json")).length === 1, "Sealed Phase 3 package must contain exactly one integrated scenario result.");
...
if (isMain) {
  try {
    const options = parsePhase3SealOptions(process.argv.slice(2));
    console.log(JSON.stringify(sealPhase3Package(options), null, 2));
  } catch (error) {
