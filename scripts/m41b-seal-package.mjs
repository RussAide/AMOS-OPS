...
import { M41B_REQUIRED_QA_STEPS } from "./m41b-run-qa.mjs";

const SCRIPT_DIRECTORY = path.dirname(fileURLToPath(import.meta.url));
const PACKAGE_BASE_NAME =
  "AMOS-OPS_M4_1B_Executive_Intelligence_Assistant_and_Workplan_Orchestration_Package_v1_0";
const SOURCE_SNAPSHOT_NAME =
  "AMOS-OPS_M4_1B_Canonical_Source_Snapshot_v1_0.zip";
const CRITERION_EVIDENCE_FILES = [
  "M4_1B_ROLE_CADENCE_WORKPLAN_MATRIX.json",
  "M4_1B_PRIORITY_GENERATION_RESULT.json",
  "M4_1B_ASK_AMOS_RESULT.json",
  "M4_1B_SOURCE_TRANSPARENCY_RESULT.json",
  "M4_1B_HUMAN_GATE_RESULT.json",
  "M4_1B_RECOMMENDATION_TASK_RESULT.json",
  "M4_1B_FIVE_CADENCE_RESULT.json",
  "M4_1B_ACCESS_REFUSAL_RESULT.json",
  "M4_1B_AUDIT_LINEAGE_RESULT.json",
  "M4_1B_PERSONA_SCENARIO_RESULT.json",
];
...
export function parseM41bSealOptions(argv) {
  let root;
  let output;
  const positional = [];
...
function walkFiles(root) {
  ...
  const visit = (directory) => {
    for (const entry of fs
      ...
      else throw new Error(`Unsupported M4.1B package entry: ${absolute}`);
    }
  };
  visit(root);
  return result;
...
function verifyQaReport(evidenceRoot) {
  const reportPath = path.join(evidenceRoot, "M4_1B_INTEGRATED_QA.json");
  assert(
    fs.existsSync(reportPath),
    `M4.1B integrated QA report is missing: ${reportPath}`,
  );
  const report = JSON.parse(fs.readFileSync(reportPath, "utf8"));
  assert(
    report.reportId === "M41B-INTEGRATED-QA" &&
      report.milestone === "M4.1B" &&
      report.evidenceClass === "synthetic_demo" &&
      report.productionRows === 0 &&
      report.usesProductionData === false,
    "M4.1B integrated QA identity or evidence boundary drifted.",
  );
  assert(report.passed === true, "M4.1B integrated QA is not passing.");
  assert(
    JSON.stringify(report.requiredSteps) ===
      JSON.stringify(M41B_REQUIRED_QA_STEPS),
    "M4.1B required QA step register drifted.",
  );
  assert(
    Array.isArray(report.focusedTests) && report.focusedTests.length > 0,
    "M4.1B QA did not retain focused test identities.",
  );
  for (const required of [
    "src/components/m41b/m41b-experience-model.test.ts",
    "src/providers/m41b-fallback-policy.test.ts",
  ])
    assert(
      report.focusedTests.includes(required),
      `M4.1B focused QA omitted ${required}.`,
    );
  assert(
    new Set(report.focusedTests).size === report.focusedTests.length,
    "M4.1B focused QA contains duplicate test execution.",
  );
  assert(
    Array.isArray(report.steps) &&
      JSON.stringify(report.steps.map((step) => step.id)) ===
        JSON.stringify(M41B_REQUIRED_QA_STEPS),
    "M4.1B executed QA step order drifted.",
  );
  assert(
    report.steps.every((step) => step.exitCode === 0),
    "M4.1B integrated QA contains a failing step.",
  );
  for (const step of report.steps) {
    assert(
      typeof step.log === "string" && !path.isAbsolute(step.log),
      `M4.1B QA log path is invalid for ${step.id}.`,
    );
    const logPath = path.resolve(evidenceRoot, step.log);
    assert(
      isWithin(evidenceRoot, logPath) && fs.existsSync(logPath),
      `M4.1B QA log is missing for ${step.id}.`,
    );
  }
  assert(
    typeof report.migrationLog === "string" &&
      !path.isAbsolute(report.migrationLog),
    "M4.1B migration verification log path is invalid.",
  );
  const migrationLog = path.resolve(evidenceRoot, report.migrationLog);
  assert(
    isWithin(evidenceRoot, migrationLog) && fs.existsSync(migrationLog),
    "M4.1B migration verification log is missing.",
  );
  const migrationText = fs.readFileSync(migrationLog, "utf8");
  for (const required of [
    '"milestone": "M4.1B"',
    '"passed": true',
    '"foreignKeyViolations": 0',
    '"m41b_interaction_events"',
    '"m41b_scenario_runs"',
    '"m41b_workplan_snapshots"',
  ])
    assert(
      migrationText.includes(required),
      `M4.1B migration log is missing proof: ${required}`,
    );
  assert(
    typeof report.migrationReport === "string" &&
      !path.isAbsolute(report.migrationReport),
    "M4.1B migration verification report path is invalid.",
  );
  ...
  assert(
    ...
    "M4.1B migration verification report is missing.",
  );
  const migrationReport = JSON.parse(
    fs.readFileSync(migrationReportPath, "utf8"),
  );
  assert(
    migrationReport.reportId === "M41B-MIGRATION-VERIFICATION" &&
      migrationReport.milestone === "M4.1B" &&
      migrationReport.evidenceClass === "synthetic_demo" &&
      migrationReport.passed === true &&
      migrationReport.databaseArtifact?.lifecycle ===
        "transient_verification_only" &&
      migrationReport.databaseArtifact?.retained === false &&
      Number.isInteger(migrationReport.databaseArtifact?.bytes) &&
      migrationReport.databaseArtifact.bytes > 0 &&
      /^[a-f0-9]{64}$/.test(migrationReport.databaseArtifact?.sha256),
    "M4.1B migration verification report or transient database hash drifted.",
  );
  return report;
}

export function sealM41bPackage(options) {
  ...
  for (const [label, target] of [
    ...
    assert(
      ...
      `M4.1B ${label} root is missing: ${target}`,
    );
  assert(
    !isWithin(sourceRoot, options.output),
    "M4.1B package output cannot be written inside the canonical source tree.",
  );
  ...
  run(
    ...
    [
      "--import",
      "tsx",
      path.join(SCRIPT_DIRECTORY, "m41b-verify-evidence.ts"),
      "--root",
      milestoneRoot,
      "--output",
      evidenceRoot,
  ...
  const acceptanceManifest = JSON.parse(
    fs.readFileSync(
      path.join(evidenceRoot, "M4_1B_ACCEPTANCE_MANIFEST.json"),
      "utf8",
    ),
  );
  assert(
    acceptanceManifest.status === "complete" &&
      acceptanceManifest.evidenceClass === "synthetic_demo",
    "Final sealing requires complete synthetic M4.1B acceptance evidence.",
  );
  assert(
    acceptanceManifest.criteriaExpected === 10 &&
      acceptanceManifest.criteriaPassed === 10,
    "Final sealing requires all 10 M4.1B criteria.",
  );
  assert(
    acceptanceManifest.exitGate === true,
    "Final sealing requires the passing M4.1B exit gate.",
  );
  ...
  for (const target of [packageZip, packageSidecar, sealResultPath])
    assert(
      ...
      `M4.1B sealing refuses to overwrite existing output: ${target}`,
    );

  const temporaryRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "amos-m41b-seal-"),
  );
  try {
    ...
    assert(
      ...
      "Filtered M4.1B canonical source snapshot is incomplete.",
    );
    assert(
      [...stagedSourceMap.keys()].every(
        (relativePath) => !sourcePathExcluded(relativePath),
      ),
      "Filtered M4.1B source still contains excluded content.",
    );
    ...
    assert(
      ...
      "M4.1B evidence contains a prohibited source, dependency, or build-output copy.",
    );
    assert(
      !evidenceFiles.some((relativePath) =>
        /\.(zip|db|sqlite|sqlite3)(-shm|-wal)?$/i.test(relativePath),
      ),
      "M4.1B evidence contains a nested snapshot or runtime database.",
    );
    const sourceEntries = walkFiles(packageSource);
    assert(
      sourceEntries.length === 1 && sourceEntries[0] === SOURCE_SNAPSHOT_NAME,
      "M4.1B package must contain exactly one canonical source snapshot and no loose source copy.",
    );

    fs.writeFileSync(
      path.join(packageRoot, "README.md"),
      `# AMOS-OPS M4.1B Integrated Sprint Package

This package contains one controlling-document set, one integrated evidence set, and one filtered canonical source snapshot for M4.1B Executive Intelligence Assistant and Workplan Orchestration.

- Evidence boundary: synthetic demonstration only
- Acceptance criteria: 10/10
- Authorized role workplans: 36
...
`,
    ...
    const checksumFiles = walkFiles(packageRoot)
      ...
      .sort((left, right) => left.path.localeCompare(right.path));
    fs.writeFileSync(
      path.join(packageManifestRoot, "M4_1B_PACKAGE_SHA256SUMS.txt"),
      `${checksumFiles
    ...
    compareMaps(
      ...
      "Integrated M4.1B package restoration",
    );
    ...
    assert(
      ...
      `Sealed M4.1B package contains ${sourceSnapshots.length} canonical source snapshots; exactly one is required.`,
    );
    assert(
      outerListing.filter((entry) =>
        entry.endsWith("/M4_1B_INTEGRATED_SCENARIO_RESULT.json"),
      ).length === 1,
      "Sealed M4.1B package must contain exactly one integrated scenario result.",
    );
    for (const criterionFile of CRITERION_EVIDENCE_FILES)
      assert(
        outerListing.filter((entry) => entry.endsWith(`/${criterionFile}`))
          .length === 1,
        `Sealed M4.1B package must contain exactly one ${criterionFile}.`,
      );
    assert(
      outerListing.filter((entry) =>
        entry.endsWith("/Verification_Logs/migration_integrity.log"),
      ).length === 1,
      "Sealed M4.1B package must contain exactly one migration verification log.",
    );
    assert(
      outerListing.filter((entry) =>
        entry.endsWith("/M4_1B_MIGRATION_VERIFICATION.json"),
      ).length === 1,
      "Sealed M4.1B package must contain exactly one migration verification report and transient database hash.",
    );
    compareMaps(
      liveSourceBefore,
...
if (isMain) {
  try {
    const options = parseM41bSealOptions(process.argv.slice(2));
    console.log(JSON.stringify(sealM41bPackage(options), null, 2));
  } catch (error) {
