...
export const groRouter = createRouter({
  ...
  listShiftLogs: authedQuery
      ...
      z
        .object({
          shiftType: z
            ...
            .optional(),
          status: z
            .enum(["active", "completed", "no_show", "absent"])
            .optional(),
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [];
      if (input?.shiftType)
        conditions.push(eq(shiftLogs.shiftType, input.shiftType));
      if (input?.status) conditions.push(eq(shiftLogs.status, input.status));
      if (input?.dateFrom)
        conditions.push(gte(shiftLogs.shiftDate, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(shiftLogs.shiftDate, input.dateTo));
  ...
  getShiftLog: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      const rounds = await db
        .select()
        .from(safetyRounds)
        .where(eq(safetyRounds.shiftId, input.id))
        .orderBy(safetyRounds.areaOrder);
      // Get related care logs
      const care = await db
        .select()
        .from(youthCareLogs)
        .where(eq(youthCareLogs.shiftId, input.id))
        .orderBy(desc(youthCareLogs.createdAt));
  ...
  addShiftLogEntry: adminQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      const log = await db
        .select({ entriesJson: shiftLogs.entriesJson })
        .from(shiftLogs)
        .where(eq(shiftLogs.id, input.id))
        .get();
  ...
  updateShiftLog: adminQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const updateData: Partial<InferInsertModel<typeof shiftLogs>> = {
        updatedAt: nowIso(),
      };
  ...
  listSafetyRounds: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      if (input?.shiftType)
        conditions.push(eq(safetyRounds.shiftType, input.shiftType));
      if (input?.dateFrom)
        conditions.push(gte(safetyRounds.shiftDate, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(safetyRounds.shiftDate, input.dateTo));
    ...
    .query(async ({ input }) => {
      ...
      return (
        db
          .select()
          .from(safetyRounds)
          .where(eq(safetyRounds.id, input.id))
          .get() ?? null
      );
    }),
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.shiftId) {
        const existing = await db
          .select()
          .from(safetyRounds)
          .where(eq(safetyRounds.shiftId, input.shiftId))
          .all();
        await db
          .update(shiftLogs)
          .set({
  ...
  updateSafetyRound: adminQuery
    ...
    .mutation(async ({ input }) => {
      ...
      if (updates.item1NoHazards !== undefined)
        updateData.item1NoHazards = updates.item1NoHazards;
      if (updates.item2LightingWorking !== undefined)
        updateData.item2LightingWorking = updates.item2LightingWorking;
      if (updates.item3EmergencyExitsClear !== undefined)
        updateData.item3EmergencyExitsClear = updates.item3EmergencyExitsClear;
      if (updates.item4FireExtinguishersOk !== undefined)
        updateData.item4FireExtinguishersOk = updates.item4FireExtinguishersOk;
      if (updates.item5NoContraband !== undefined)
        updateData.item5NoContraband = updates.item5NoContraband;
      if (updates.item6CleanSanitary !== undefined)
        updateData.item6CleanSanitary = updates.item6CleanSanitary;
      if (updates.item7EquipmentSecure !== undefined)
        updateData.item7EquipmentSecure = updates.item7EquipmentSecure;
      if (updates.item8YouthAreasSafe !== undefined)
        updateData.item8YouthAreasSafe = updates.item8YouthAreasSafe;
      if (updates.hazardsFound !== undefined)
        updateData.hazardsFound = updates.hazardsFound;
      if (updates.correctiveAction !== undefined)
        updateData.correctiveAction = updates.correctiveAction;
      if (updates.requiresFollowUp !== undefined)
        updateData.requiresFollowUp = updates.requiresFollowUp;
      if (updates.followUpNotes !== undefined)
        updateData.followUpNotes = updates.followUpNotes;
      if (updates.reviewedBy !== undefined) {
        updateData.reviewedBy = updates.reviewedBy;
        updateData.reviewedAt = nowIso();
      }
  ...
  safetyRoundDashboard: authedQuery.query(async () => {
    ...
    const followUpNeeded = all.filter(
      (r) => r.requiresFollowUp && !r.reviewedAt,
    ).length;
    ...
    return {
      ...
      followUpNeeded,
      completionRate:
        all.length > 0 ? Math.round((totalCompleted / all.length) * 100) : 0,
      byArea,
    };
  }),
  ...
  listYouthCareLogs: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [];
      if (input?.youthId)
        conditions.push(eq(youthCareLogs.youthId, input.youthId));
      if (input?.careType)
        conditions.push(eq(youthCareLogs.careType, input.careType));
      if (input?.dateFrom)
        conditions.push(gte(youthCareLogs.logDate, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(youthCareLogs.logDate, input.dateTo));
    ...
    .query(async ({ input }) => {
      ...
      return (
        db
          .select()
          .from(youthCareLogs)
          .where(eq(youthCareLogs.id, input.id))
          .get() ?? null
      );
    }),
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.shiftId) {
        const existing = await db
          .select()
          .from(youthCareLogs)
          .where(eq(youthCareLogs.shiftId, input.shiftId))
          .all();
        await db
          .update(shiftLogs)
          .set({
  ...
  updateYouthCareLog: adminQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      const { id, ...updates } = input;
      const updateData: Partial<InferInsertModel<typeof youthCareLogs>> = {
        updatedAt: nowIso(),
      };
      ...
      if (updates.youthResponse !== undefined)
        updateData.youthResponse = updates.youthResponse;
      if (updates.outcome !== undefined) updateData.outcome = updates.outcome;
      if (updates.followUpNeeded !== undefined)
        updateData.followUpNeeded = updates.followUpNeeded;
      if (updates.followUpActions !== undefined)
        updateData.followUpActions = updates.followUpActions;
      if (updates.reviewedBy !== undefined) {
        updateData.reviewedBy = updates.reviewedBy;
        updateData.reviewedAt = nowIso();
      }
      ...
      await db
        .update(youthCareLogs)
        .set(updateData)
        .where(eq(youthCareLogs.id, id));
      return { success: true };
    }),
  ...
  listIncidentReports: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [];
      if (input?.status)
        conditions.push(eq(incidentReports.status, input.status));
      if (input?.incidentType)
        conditions.push(eq(incidentReports.incidentType, input.incidentType));
      if (input?.severity)
        conditions.push(eq(incidentReports.severity, input.severity));
      if (input?.youthId)
        conditions.push(eq(incidentReports.youthId, input.youthId));
      if (input?.dateFrom)
        conditions.push(gte(incidentReports.occurredAt, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(incidentReports.occurredAt, input.dateTo));
  ...
  getIncidentReport: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      const supervision = await db
        .select()
        .from(supervisionNotes)
        .where(eq(supervisionNotes.relatedIncidentId, input.id))
        .all();
  ...
  updateIncidentReport: adminQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      const { id, ...updates } = input;
      const updateData: Partial<InferInsertModel<typeof incidentReports>> = {
        updatedAt: nowIso(),
      };
      ...
      if (updates.description !== undefined)
        updateData.description = updates.description;
      if (updates.immediateAction !== undefined)
        updateData.immediateAction = updates.immediateAction;
      if (updates.factors !== undefined) updateData.factors = updates.factors;
      if (updates.youthInjuries !== undefined)
        updateData.youthInjuries = updates.youthInjuries;
      if (updates.staffInjuries !== undefined)
        updateData.staffInjuries = updates.staffInjuries;
      if (updates.medicalAttentionRequired !== undefined)
        updateData.medicalAttentionRequired = updates.medicalAttentionRequired;
      if (updates.medicalAttentionProvided !== undefined)
        updateData.medicalAttentionProvided = updates.medicalAttentionProvided;
      if (updates.guardianNotified !== undefined) {
        updateData.guardianNotified = updates.guardianNotified;
        if (updates.guardianNotified) updateData.guardianNotifiedAt = nowIso();
      }
      if (updates.guardianNotifiedBy !== undefined)
        updateData.guardianNotifiedBy = updates.guardianNotifiedBy;
      if (updates.supervisorNotified !== undefined) {
        updateData.supervisorNotified = updates.supervisorNotified;
        if (updates.supervisorNotified)
          updateData.supervisorNotifiedAt = nowIso();
      }
      if (updates.supervisorNotifiedBy !== undefined)
        updateData.supervisorNotifiedBy = updates.supervisorNotifiedBy;
      if (updates.investigatorAssigned !== undefined)
        updateData.investigatorAssigned = updates.investigatorAssigned;
      if (updates.investigationNotes !== undefined)
        updateData.investigationNotes = updates.investigationNotes;
      if (updates.rootCause !== undefined)
        updateData.rootCause = updates.rootCause;
      if (updates.correctiveActions !== undefined)
        updateData.correctiveActions = updates.correctiveActions;
  ...
  incidentDashboard: authedQuery.query(async () => {
    ...
    const pendingSupervisor = all.filter(
      (r) => r.status === "pending_supervisor",
    ).length;
    ...
    const high = all.filter(
      (r) => r.severity === "high" && r.status !== "closed",
    ).length;
  ...
  listSupervisionNotes: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      if (input?.superviseeId)
        conditions.push(eq(supervisionNotes.superviseeId, input.superviseeId));
      if (input?.supervisionType)
        conditions.push(
          eq(supervisionNotes.supervisionType, input.supervisionType),
        );
      if (input?.dateFrom)
        conditions.push(gte(supervisionNotes.supervisionDate, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(supervisionNotes.supervisionDate, input.dateTo));
      ...
      let results =
        conditions.length > 0
          ? await db
              ...
              .from(supervisionNotes)
              .where(and(...conditions))
              .orderBy(desc(supervisionNotes.createdAt))
          : await db
              .select()
              .from(supervisionNotes)
              .orderBy(desc(supervisionNotes.createdAt));
      ...
      if (input?.followUpRequired) {
        results = results.filter(
          (r) => r.followUpRequired && !r.superviseeAcknowledged,
        );
      }
      return results;
    }),
  ...
  updateSupervisionNote: adminQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      const { id, ...updates } = input;
      const updateData: Partial<InferInsertModel<typeof supervisionNotes>> = {
        updatedAt: nowIso(),
      };
      if (updates.topicsDiscussed !== undefined)
        updateData.topicsDiscussed = updates.topicsDiscussed;
      if (updates.staffConcerns !== undefined)
        updateData.staffConcerns = updates.staffConcerns;
      if (updates.performanceObservations !== undefined)
        updateData.performanceObservations = updates.performanceObservations;
      if (updates.trainingNeeds !== undefined)
        updateData.trainingNeeds = updates.trainingNeeds;
      if (updates.goalsSet !== undefined)
        updateData.goalsSet = updates.goalsSet;
      if (updates.actionItems !== undefined)
        updateData.actionItems = updates.actionItems;
      if (updates.followUpRequired !== undefined)
        updateData.followUpRequired = updates.followUpRequired;
      if (updates.followUpDate !== undefined)
        updateData.followUpDate = updates.followUpDate;
      if (updates.followUpTopics !== undefined)
        updateData.followUpTopics = updates.followUpTopics;

      await db
        .update(supervisionNotes)
        .set(updateData)
        .where(eq(supervisionNotes.id, id));
      return { success: true };
    }),
  ...
  listShiftHandoffs: authedQuery
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [];
      if (input?.status)
        conditions.push(eq(shiftHandoffs.status, input.status));
      if (input?.dateFrom)
        conditions.push(gte(shiftHandoffs.handoffDate, input.dateFrom));
      if (input?.dateTo)
        conditions.push(lte(shiftHandoffs.handoffDate, input.dateTo));
  ...
  updateShiftHandoff: adminQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      const { id, ...updates } = input;
      const updateData: Partial<InferInsertModel<typeof shiftHandoffs>> = {
        updatedAt: nowIso(),
      };
      if (updates.youthStatusJson !== undefined)
        updateData.youthStatusJson = updates.youthStatusJson;
      if (updates.pendingItems !== undefined)
        updateData.pendingItems = updates.pendingItems;
      if (updates.medicationUpdates !== undefined)
        updateData.medicationUpdates = updates.medicationUpdates;
      if (updates.appointmentReminders !== undefined)
        updateData.appointmentReminders = updates.appointmentReminders;
      if (updates.highPriorityAlerts !== undefined)
        updateData.highPriorityAlerts = updates.highPriorityAlerts;
      if (updates.safetyAlerts !== undefined)
        updateData.safetyAlerts = updates.safetyAlerts;
      if (updates.generalNotes !== undefined)
        updateData.generalNotes = updates.generalNotes;
  ...
  residentialDashboard: authedQuery.query(async () => {
    ...
    const safetyFollowUp = allSafety.filter(
      (s) => s.requiresFollowUp && !s.reviewedAt,
    ).length;

    // Care logs
    const allCare = await db.select().from(youthCareLogs);
    const todayCare = allCare.filter((c) => c.logDate === today).length;
    const careFollowUp = allCare.filter(
      (c) => c.followUpNeeded && !c.reviewedBy,
    ).length;
    ...
    const pendingAck = allSupervision.filter(
      (s) => !s.superviseeAcknowledged,
    ).length;

    // Handoffs
    const allHandoffs = await db.select().from(shiftHandoffs);
    const pendingHandoffs = allHandoffs.filter(
      (h) => h.status === "pending",
    ).length;
    ...
    return {
      ...
      safetyRounds: {
        total: allSafety.length,
        today: todaySafety,
        followUpNeeded: safetyFollowUp,
      },
      youthCareLogs: {
        total: allCare.length,
        today: todayCare,
        followUpNeeded: careFollowUp,
      },
      incidents: {
  ...
  seedResidentialData: adminQuery.mutation(async () => {
        ...
        entriesJson: JSON.stringify([
          {
            ...
            note: "Shift started. Full coverage.",
          },
          {
            time: `${today}T08:30:00Z`,
            category: "medication",
            note: "Morning med pass completed.",
          },
          {
            time: `${today}T10:00:00Z`,
            category: "safety_round",
            note: "Morning safety round completed.",
          },
        ]),
    ...
    const careLogsData = [
      {
        ...
        mrn: "SYNTH-REC-001",
        careType: "daily_living" as const,
        description:
          "Assisted with morning hygiene routine. Youth cooperative and in good mood.",
      },
      {
        youthId: "youth-002",
        youthName: "Synthetic Youth 005",
        mrn: "SYNTH-REC-002",
        careType: "educational" as const,
        description:
          "Completed math homework with tutoring support. Engaged well with material.",
      },
      {
        youthId: "youth-003",
        youthName: "Synthetic Youth 014",
        mrn: "SYNTH-REC-003",
        careType: "behavioral" as const,
        description:
          "Participated in coping skills group. Practiced breathing exercises.",
      },
    ];
    for (const cl of careLogsData) {
      await db
        .insert(youthCareLogs)
        .values({
          id: randomUUID(),
          youthId: cl.youthId,
    ...
    await db
      ...
      .values({
        ...
        description:
          "Youth became frustrated during group activity and knocked over a chair. De-escalated within 5 minutes using verbal redirection. No injuries.",
        immediateAction:
          "Removed youth to quiet room. Provided space and time. Checked in after 10 minutes.",
    ...
    await db
      ...
      .values({
        ...
        topicsDiscussed:
          "Reviewed crisis intervention techniques. Discussed strategies for supporting youth with trauma history.",
        staffConcerns: "Feeling overwhelmed with new youth admissions.",
        performanceObservations:
          "Consistently demonstrates empathy and patience. Documentation is thorough.",
        ...
        actionItems: JSON.stringify([
          { task: "Enroll in TIC training", due: "2026-07-15" },
          { task: "Shadow senior RCS on intake", due: "2026-07-10" },
        ]),
        followUpRequired: true,
        followUpDate: `${today.split("-")[0]}-${today.split("-")[1]}-15`,
        followUpTopics:
          "Check progress on TIC training enrollment. Review intake shadow experience.",
    ...
    await db
      ...
      .values({
        ...
        youthStatusJson: JSON.stringify([
          ...
          {
            youthId: "youth-002",
            name: "Synthetic Youth 005",
            status: "stable",
            concerns: "Guardian visit scheduled tomorrow",
          },
          {
            youthId: "youth-003",
            name: "Synthetic Youth 014",
            status: "monitoring",
            concerns: "Behavioral incident at 14:30 - follow up recommended",
          },
        ]),
        pendingItems:
          "Guardian visit for Synthetic-Person-005 tomorrow 10am; Synthetic-Person-014 behavioral follow-up; Med pass log review",
        ...
        highPriorityAlerts:
          "Synthetic Youth 014 - behavioral incident requires follow-up in evening shift",
        safetyAlerts: "None",
        generalNotes:
          "Quiet day overall. All safety rounds completed. 3 care logs entered.",
        status: "completed",
    ...
    return {
      success: true,
      message:
        "GRO residential data seeded: 1 shift log, 8 safety rounds, 3 care logs, 1 incident, 1 supervision note, 1 handoff",
    };
  }),
});
