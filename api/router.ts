...
import { phase3Router } from "./routers/phase3";
import { m41aRouter } from "./routers/m41a";
import { m41bRouter } from "./routers/m41b";
import { m41cRouter } from "./routers/m41c";
import { m42Router } from "./routers/m42";
import { m51aRouter } from "./routers/m51a";
import { m51bRouter } from "./routers/m51b";
import { m52Router } from "./routers/m52";
import { dx1Router } from "./routers/dx1";
import { m23Router } from "./routers/m23";
import { m22Router } from "./routers/m22";
import { m24GroRouter } from "./routers/m24-gro";
...
import {
  ...
  mgmaDomains,
  mgmaKpiTargets,
  sudRecords as sudRecordsTable,
  part2Consents,
  qsoaAgreements,
  part2AuditLog,
  dmsDocuments,
  nilEntities as nilEntitiesTable,
  nilRelationships,
  hrPeople,
  incidents,
  correctiveActions,
  ...
  trainingModules,
  trainingProgress,
  separationChecklists,
  shifts,
} from "@db/schema";
import { eq } from "drizzle-orm";

// ─── BHC CLINICAL (M5: AMOS-Clinical) ──────────────────────
const clinicalRouter = createRouter({
  ...m5Router._def.record,
  ...bhcRouter._def.record,
});
...
export const appRouter = createRouter({
  ...
  forms: formsRouter,
  email: emailRouter,
  regulatoryFramework: regulatoryFrameworkRouter,
  ...
  dashboard: createRouter({
    overview: publicQuery.query(async () => {
      ...
      for (const s of allSessions) {
        ...
        try {
          ...
          if (
            risk.suicideRisk === "high" ||
            risk.homocideRisk === "high" ||
            risk.elopementRisk === "high"
          )
            highRiskCount++;
        } catch {
          /* ignore */
      ...
      const totalCollected = allClaims.reduce(
      ...
      const totalBeds = facilityRows.reduce(
        (sum, f) => sum + (f.operationalCapacity ?? f.totalBeds ?? 0),
        0,
      );
      ...
      for (const k of mgmaKPIRows) {
        if (!kpiByDomain[k.domainId])
          kpiByDomain[k.domainId] = { total: 0, onTarget: 0 };
        kpiByDomain[k.domainId].total++;
        if (k.status === "on_target" || k.status === "at_risk")
          kpiByDomain[k.domainId].onTarget++;
      }

      // Part 2
      const sudRecordRows = await db.select().from(sudRecordsTable).all();
      const activeSUD = sudRecordRows.filter(
        (r) => r.status === "active",
      ).length;
      const consentRows = await db.select().from(part2Consents).all();
      const validConsents = consentRows.filter(
        (c) => c.status === "active",
      ).length;
      const expiredConsents = consentRows.filter(
        (c) => c.status === "expired",
      ).length;
      ...
      return {
        ...
        revenue: {
          ...
          appealedClaims: allClaims.filter((c) => c.status === "appealed")
            .length,
        },
        campus: {
          totalBeds,
        ...
        mgma: {
          domainCount: mgmaDomainRows.length,
          kpiCount: mgmaKPIRows.length,
          domains: mgmaDomainRows.map((d) => ({
            id: d.id,
            name: d.domainName,
            number: d.domainNumber,
            progress: kpiByDomain[d.id]
              ? Math.round(
                  (kpiByDomain[d.id].onTarget / kpiByDomain[d.id].total) * 100,
                )
              : 0,
          })),
        },
        part2: {
          activeSUDRecords: activeSUD,
          totalSUDRecords: sudRecordRows.length,
          validConsents,
          expiredConsents,
          activeQSOAs: activeQSOA,
          totalQSOAs: qsoaRows.length,
          recentAudits: auditLogEntries.length,
        },
        documents: {
        ...
        nil: {
          entityCount: nilEntityRows.length,
          relationshipCount: nilRelationRows.length,
        },
      };
    }),
    ...
    operationalKPIs: publicQuery.query(async () => {
      ...
      const openCases = allPlans.filter(
      ...
      const pendingDocumentation = allSessions.filter(
        (s) =>
          s.status === "completed" &&
          (!s.sessionNotes || s.sessionNotes.length < 10),
      ).length;

      // 4. Staff on duty from shifts (today's shifts that are in_progress)
      const shiftRows = await db.select().from(shifts).all();
      const todayShifts = shiftRows.filter(
        (s) =>
          s.shiftDate === todayStr &&
      ...
      const staffOnDuty = todayShifts.reduce((sum, s) => {
        ...
        if (s.rcsStaffIdsJson) {
          try {
            const ids = JSON.parse(s.rcsStaffIdsJson);
      ...
      const openIncidents = allIncidentsRows.filter(
    ...
    complianceKPIs: publicQuery.query(async () => {
      ...
      const upcomingAudits = allAudits.filter(
      ...
      const overdueCAPs = allCaps.filter(
        (a) =>
          a.status === "overdue" ||
          (a.dueDate && a.dueDate < iso && a.status !== "completed"),
      ).length;
      ...
      const avgAuditScore =
        completedAudits.length > 0
          ? Math.round(
              completedAudits.reduce((s, a) => s + (a.score ?? 0), 0) /
                completedAudits.length,
            )
          : 0;
      const complianceScore =
        avgAuditScore > 0 ? avgAuditScore : Math.round(100 - overdueItems * 5);
      ...
      const expiringCredentials = allCredentials.filter((c) => {
        ...
        const days = Math.ceil(
          (new Date(c.expiryDate).getTime() - Date.now()) / 86400000,
        );
        return (
          c.status === "expiring" ||
          (c.status === "valid" && days <= 90 && days > 0)
        );
      }).length;

      // 6. Incident count 30d from incidents
      const allIncidentsRows = await db.select().from(incidents).all();
      const incidentCount30d = allIncidentsRows.filter(
    ...
    clinicalKPIs: publicQuery.query(async () => {
      ...
      const activePatients = allPatients.filter(
        (p) => p.status === "active" && p.intakeDate,
      );
      const avgLOS =
        activePatients.length > 0
          ? Math.round(
              activePatients.reduce((sum, p) => {
                const days = Math.ceil(
                  (now.getTime() - new Date(p.intakeDate!).getTime()) /
                    86400000,
                );
                return sum + days;
      ...
      const readmissions = dischargedPatients.filter((p) => {
        ...
        return (
          reIntakeTime > dischargeTime &&
          reIntakeTime - dischargeTime < 30 * 86400000
        );
      }).length;
      const readmissionRate =
        dischargedPatients.length > 0
          ? Math.round((readmissions / dischargedPatients.length) * 100)
          : 0;
      ...
      const approvedAuths = allAuths.filter(
        (a) => a.status === "approved",
      ).length;
      const authorizationStatus =
        allAuths.length > 0
          ? Math.round((approvedAuths / allAuths.length) * 100)
          : 0;

      return {
        avgLOS,
        planCompletionRate,
        outcomeMeasureTrends: recentOutcomes,
        readmissionRate,
    ...
    revenueKPIs: publicQuery.query(async () => {
      ...
      const allClaims = await db.select().from(claimsTable).all();
      const claimsSubmitted30d = allClaims.filter(
        (c) => c.submissionDate && c.submissionDate >= thirtyDaysAgo,
      ).length;

      // 2. Approval rate
      const decidedClaims = allClaims.filter(
        (c) =>
          c.status === "approved" ||
          c.status === "paid" ||
          c.status === "denied",
      ...
      const approvalRate =
        decidedClaims.length > 0
          ? Math.round((approvedClaims / decidedClaims.length) * 100)
          : 0;
      ...
      const denialRate =
        decidedClaims.length > 0
          ? Math.round((deniedClaims / decidedClaims.length) * 100)
          : 0;

      // 4. Avg days to payment (for paid claims)
      const paidClaims = allClaims.filter(
        (c) => c.status === "paid" && c.submissionDate,
      );
        ...
        paidClaims.length > 0
          ? Math.round(
              paidClaims.reduce((sum, c) => {
                ...
                const paid = c.updatedAt
                  ? new Date(c.updatedAt).getTime()
                  : now.getTime();
                return (
                  sum + Math.max(0, Math.ceil((paid - submitted) / 86400000))
                );
              }, 0) / paidClaims.length,
            )
          : 0;
      ...
      const outstandingAR = allClaims
        .filter((c) =>
          ["submitted", "acknowledged", "pending_review", "approved"].includes(
            c.status,
          ),
        )
    ...
    workforceKPIs: publicQuery.query(async () => {
      ...
      const openPositions = allPeople.filter(
        (p) => !p.isEmployee && p.isActive,
      ).length;

      // 3. Credential compliance rate
      const allCredentials = await db.select().from(credentialsTable).all();
      const validCredentials = allCredentials.filter(
        (c) => c.status === "valid",
      ).length;
      const credentialComplianceRate =
        allCredentials.length > 0
          ? Math.round((validCredentials / allCredentials.length) * 100)
          : 0;
      ...
      const completedTraining = allProgress.filter(
        (p) => p.status === "completed",
      ).length;
      const totalTrainingAssignments = allProgress.length || allModules.length;
      const trainingCompletionRate =
        totalTrainingAssignments > 0
          ? Math.round((completedTraining / totalTrainingAssignments) * 100)
          : 0;
      ...
      const turnoverRate12m =
        totalStaff > 0
          ? Math.round(
              (formerlyEmployed / (totalStaff + formerlyEmployed)) * 100,
            )
          : 0;
    ...
    executiveKPIs: publicQuery.query(async () => {
      ...
      const expiredCreds = allCredentials.filter(
        (c) => c.status === "expired",
      ).length;
      const compliancePosture = Math.max(
        0,
        Math.min(100, avgAuditScore - overdueCAPs * 5 - expiredCreds * 3),
      );
      ...
      const allPeople = await db.select().from(hrPeople).all();
      const activeStaff = allPeople.filter(
        (p) => p.isEmployee && p.isActive,
      ).length;
      const todayStr = iso.split("T")[0];
      const shiftRows = await db.select().from(shifts).all();
      ...
      const staffingLevel =
        activeStaff > 0
          ? Math.round((staffOnDuty / Math.max(activeStaff, 1)) * 100)
          : 0;
      ...
      const strategicProjectStatus =
        domainProgress.length > 0
          ? Math.round(
              domainProgress.reduce((s, d) => s + d.progress, 0) /
