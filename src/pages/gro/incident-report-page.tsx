...
export default function IncidentReportPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = trpc.groResidential.listIncidentReports.useQuery(
  ...
  const updateReport = trpc.groResidential.updateIncidentReport.useMutation({
    onSuccess: () => {
      utils.groResidential.listIncidentReports.invalidate();
      utils.groResidential.getIncidentReport.invalidate();
      utils.groResidential.incidentDashboard.invalidate();
  ...
  const filtered = reports.filter((r) => {
    ...
    return (
      !q ||
      r.youthName?.toLowerCase().includes(q) ||
  ...
  const severityColor = (s: string) =>
    SEVERITY_OPTIONS.find((o) => o.value === s)?.color ??
    "bg-gray-100 text-gray-700";
  const statusColor = (s: string) =>
    STATUS_OPTIONS.find((o) => o.value === s)?.color ??
    "bg-gray-100 text-gray-700";
  ...
  return (
    <PageLayout
      ...
      subtitle="Residential incident reporting and tracking"
    >
      <div className="px-4 md:px-6 pt-4 pb-8 max-w-7xl mx-auto space-y-6">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs uppercase tracking-wider text-gray-500 flex items-center gap-1.5">
                <FileWarning className="h-3.5 w-3.5" /> Total
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilterSeverity(value as typeof filterSeverity)
            }
          >
        ...
        {isLoading ? (
          ...
          <Card className="border-red-200 bg-red-50">
            <CardContent className="p-8 text-center text-sm text-red-800">
              Connected incident data is unavailable. No demonstration records
              were substituted.
            </CardContent>
          </Card>
        ) : filtered.length === 0 ? (
                ...
                {filtered.map((r) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={cn("text-[10px]", severityColor(r.severity))}
                      >
                        {r.severity}
                      </Badge>
                    </TableCell>
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Report a residential incident. All reports are timestamped.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  id="med-attn"
                  checked={formMedical}
                  onChange={(e) => setFormMedical(e.target.checked)}
                />
                <Label
                  htmlFor="med-attn"
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() =>
                  createReport.mutate({
                    incidentType: formType as Parameters<
                      typeof createReport.mutate
                    >[0]["incidentType"],
                    severity: formSeverity as Parameters<
                      typeof createReport.mutate
                    >[0]["severity"],
        ...
        >
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {detail &&
                  `${detail.occurredAt?.split("T")[0]} · ${detail.occurredLocation}`}
              </DialogDescription>
            </DialogHeader>
            {detail && (
              <Tabs
              ...
              >
                ...
                <TabsContent value="overview" className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-xs text-gray-500">Youth</span>
                    ...
                    <div>
                      ...
                      <div>{detail.occurredLocation}</div>
                    </div>
                    <div>
                    ...
                    {detail.witnesses && (
                      <div>
                        <span className="text-xs text-gray-500">Witnesses</span>
                        <div>{detail.witnesses}</div>
                      </div>
                    )}
                  </div>
                  <Separator />
                  ...
                  {detail.factors && (
                    <div>
                      ...
                      <p className="text-sm mt-1">{detail.factors}</p>
                    </div>
                  )}
                  {detail.medicalAttentionRequired && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-2 text-red-700 text-sm">
                      <Stethoscope className="h-4 w-4" /> Medical attention was
                      required
                    </div>
                  )}
                  {detail.youthInjuries && (
                    <div>
                  ...
                  {(detail.status === "resolved" ||
                    detail.status === "pending_supervisor") && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() =>
                ...
                <TabsContent value="investigation" className="space-y-4">
                  <div>
                    ...
                    <Input
                      defaultValue={detail.investigatorAssigned ?? ""}
                      onBlur={(e) => {
                        if (e.target.value)
                          updateReport.mutate({
                            id: detail.id,
                            investigatorAssigned: e.target.value,
                          });
                      }}
                  ...
                  <div>
                    ...
                    <Textarea
                      defaultValue={detail.investigationNotes ?? ""}
                      onBlur={(e) => {
                        if (e.target.value)
                          updateReport.mutate({
                            id: detail.id,
                  ...
                  <div>
                    ...
                    <Textarea
                      defaultValue={detail.correctiveActions ?? ""}
                      onBlur={(e) => {
                        if (e.target.value)
                          updateReport.mutate({
                            id: detail.id,
                  ...
                  {detail.supervisionNotes &&
                    detail.supervisionNotes.length > 0 && (
                      <div>
                        <Label className="text-xs text-gray-500">
                          Related Supervision Notes (
                          {detail.supervisionNotes.length})
                        </Label>
                        {detail.supervisionNotes.map((sn) => (
                          <div
                            key={sn.id}
                            className="text-xs p-2 border rounded mt-1"
                          >
                            <span className="font-medium">
                              {sn.supervisionType}
                            </span>{" "}
                            — {sn.topicsDiscussed?.substring(0, 80)}...
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 border rounded">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Guardian Notified</span>
                      </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (confirm("Delete this incident report?"))
                    deleteReport.mutate({ id: selectedId! });
                }}
              >
