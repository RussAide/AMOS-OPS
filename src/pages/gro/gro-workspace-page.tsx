...
export default function GROWorkspacePage() {
  ...
  const { data: incidentDashboard, isError: incidentsUnavailable } =
    trpc.groResidential.incidentDashboard.useQuery();
  const { data: safetyDashboard, isError: safetyUnavailable } =
  ...
  return (
    <PageLayout
      ...
      title="GRO Residential Operations"
      subtitle="Comprehensive residential operations management"
    >
      <div className="px-4 md:px-6 pt-4 pb-8 max-w-7xl mx-auto space-y-6">
        {(dashboardUnavailable ||
          ...
          <Card className="border-red-200 bg-red-50">
            <CardContent className="p-4 text-sm text-red-800">
              Connected residential operations data is temporarily unavailable.
              No demonstration records were substituted.
            </CardContent>
          </Card>
        )}
        ...
        {(criticalIncidents > 0 || openIncidents > 0) && (
          ...
          >
            ...
            <Button
              ...
              onClick={() => navigate("/gro/incidents")}
            >
        ...
        {(safetyFollowUp > 0 || careFollowUp > 0) && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm bg-amber-50 border border-amber-200 text-amber-800">
            ...
            <div className="flex-1">
              <span className="font-semibold">
                {safetyFollowUp > 0
                  ? `${safetyFollowUp} safety round${safetyFollowUp > 1 ? "s" : ""} need follow-up`
                  : ""}
                {safetyFollowUp > 0 && careFollowUp > 0 ? " · " : ""}
                {careFollowUp > 0
                  ? `${careFollowUp} care log${careFollowUp > 1 ? "s" : ""} need follow-up`
                  : ""}
              </span>
            </div>
            <div className="flex gap-1.5">
              {safetyFollowUp > 0 && (
                <Button
                  ...
                  onClick={() => navigate("/gro/safety-rounds")}
                >
                  Safety <ChevronRight className="h-3 w-3 ml-0.5" />
                </Button>
              )}
              {careFollowUp > 0 && (
                <Button
                  ...
                  variant="outline"
                  className="text-xs h-7 border-amber-300 text-amber-700 hover:bg-amber-100"
                  onClick={() => navigate("/gro/care-logs")}
                >
                  Care <ChevronRight className="h-3 w-3 ml-0.5" />
                </Button>
              )}
            </div>
          </div>
          ...
          {FEATURE_CARDS.map((feature) => {
            ...
            return (
              <Card
                ...
                style={{
                  borderLeftColor: feature.color.includes("green")
                    ? "#22c55e"
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className={cn("text-[10px]", feature.color)}
                    >
        ...
        {/* Demonstration controls never render in the operational workspace. */}
          ...
          <div className="flex justify-end">
            ...
            >
              <RefreshCw
                className={cn(
                  "h-4 w-4 mr-1.5",
                  seedData.isPending && "animate-spin",
                )}
              />
              {seedData.isPending ? "Seeding..." : "Seed Demonstration Data"}
            </Button>
          </div>
        )}
      </div>
