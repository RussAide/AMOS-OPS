...
export default function SafetyRoundPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = trpc.groResidential.listSafetyRounds.useQuery(
  ...
  const { data: dashboard } =
    trpc.groResidential.safetyRoundDashboard.useQuery();
  ...
  const allChecked = () =>
    CHECKLIST_ITEMS.every((item) => formChecks[item.key]);

  return (
    <PageLayout
      category="Residential"
      title="Safety Round Checklists"
      subtitle="Area-based safety inspection checklists"
    >
      <div className="px-4 md:px-6 pt-4 pb-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs uppercase tracking-wider text-gray-500 flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5" /> Total
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-blue-700">
                {dashboard?.totalRounds ?? 0}
              </div>
              <p className="text-xs text-gray-500">Safety rounds</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-500">All items passed</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs uppercase tracking-wider text-gray-500 flex items-center gap-1.5">
                <ClipboardCheck className="h-3.5 w-3.5" /> Completion
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-[#2e8b8b]">
                {dashboard?.completionRate ?? 0}%
              </div>
              <div className="mt-1 w-full bg-gray-100 rounded-full h-1.5">
                <div
                  className="bg-[#2e8b8b] h-1.5 rounded-full"
        ...
        {dashboard?.byArea && Object.keys(dashboard.byArea).length > 0 && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold">By Area</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input
              placeholder="Search safety rounds..."
              value={search}
          ...
          >
            <Plus className="h-4 w-4 mr-1.5" /> New Round
          </Button>
        ...
        {isLoading ? (
          ...
          <Card className="border-red-200 bg-red-50">
            <CardContent className="p-8 text-center text-sm text-red-800">
              Connected safety-round data is unavailable. No demonstration
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete a safety round checklist for an area.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormCompletedBy(e.target.value)}
              ...
              <div>
                ...
                <div className="mt-2 flex items-center gap-2 text-xs">
                  <CheckCircle2
                    className={cn(
                      "h-4 w-4",
                      allChecked() ? "text-green-500" : "text-gray-300",
                    )}
                  />
                  <span
                    className={
                  ...
                  >
                    {Object.values(formChecks).filter(Boolean).length} of{" "}
                    {CHECKLIST_ITEMS.length} checked
                  </span>
                </div>
              </div>
              <div>
              ...
              <div className="flex items-start gap-2">
                <Checkbox
                  id="follow-up"
                  checked={formRequiresFollowUp}
                  onCheckedChange={(c) => setFormRequiresFollowUp(c === true)}
                />
                <Label
                  htmlFor="follow-up"
                  className="text-sm font-normal cursor-pointer"
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() =>
                  createRound.mutate({
                    ...
                    shiftType: formShiftType as Parameters<
                      typeof createRound.mutate
                    >[0]["shiftType"],
                    area: formArea,
                    completedBy: formCompletedBy,
                    item1NoHazards: formChecks["item1NoHazards"],
                    item2LightingWorking: formChecks["item2LightingWorking"],
                    item3EmergencyExitsClear:
                      formChecks["item3EmergencyExitsClear"],
                    item4FireExtinguishersOk:
                      formChecks["item4FireExtinguishersOk"],
                ...
                disabled={
                  !formArea || !formCompletedBy || createRound.isPending
                }
        ...
        >
          <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {detail && `${detail.shiftDate} · ${detail.area}`}
              </DialogDescription>
            </DialogHeader>
            {detail && (
              <div className="space-y-4">
                ...
                {detail.hazardsFound && (
                  <div>
                    ...
                    <p className="text-sm mt-1 bg-red-50 p-2 rounded text-red-800">
                      {detail.hazardsFound}
                    </p>
                  </div>
                )}
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    ...
                    {detail.reviewedAt && (
                      <p className="text-xs text-green-600 mt-1">
                        Reviewed by {detail.reviewedBy} on{" "}
            ...
            <DialogFooter>
              <Button
                ...
                onClick={() => {
                  if (confirm("Delete?"))
                    deleteRound.mutate({ id: selectedId! });
                }}
              >
