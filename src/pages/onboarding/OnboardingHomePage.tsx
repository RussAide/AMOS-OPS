...
import { TrackSelectionGrid } from "@/components/onboarding/TrackSelectionGrid";
...
export function OnboardingHomePage() {
  ...
  const pendingEvidence = evidence.filter((e) => e.status === "pending" || e.status === "reviewing").length;
  const inProgressEmployees = employees.filter((e) => e.clearanceStatus === "in-progress").length;
    ...
    <div>
        ...
        {statCards.map((stat) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-[22px] font-bold mb-0.5" style={{ color: "var(--topbar-title)" }}>
                {stat.value}
              </p>
              <p className="text-[12px] font-medium" style={{ color: stat.color }}>
                {stat.label}
              </p>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[18px] font-semibold flex items-center gap-2" style={{ color: "var(--topbar-title)" }}>
            <BookOpen size={20} style={{ color: "#245C5A" }} />
            Select Your Track
          </h2>
          <span className="text-[12px] font-medium px-2 py-1 rounded" style={{ backgroundColor: "#F1F5F9", color: "#475569" }}>
