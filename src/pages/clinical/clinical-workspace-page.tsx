 ...
 *  - D007-04: Synthetic legacy measure preview with no clinical record write
 */
...
type QuickActionKey =
  "admit" | "session" | "soap" | "outcome" | "treatment-plan";

type OutcomeMeasureType = "PHQ-A" | "PCL-5";
type PatientGender = "male" | "female" | "non_binary" | "prefer_not_say";

export const M41C_LEGACY_OUTCOME_MEASURE_PREVIEW_ENABLED = false as const;
...
const OUTCOME_MEASURES: OutcomeMeasureDef[] = [
  {
    ...
    description:
      "Patient Health Questionnaire for Adolescents — 13 items assessing depression severity.",
    questions: [
      {
        id: "phqa-1",
  ...
  },
  {
    ...
    name: "PCL-5 (PTSD)",
    description:
      "PTSD Checklist for DSM-5 — 20 items assessing PTSD symptom severity.",
    questions: [
      {
        id: "pcl-1",
      ...
      {
        id: "pcl-3",
        text: "Suddenly feeling or acting as if the stressful experience were happening again",
        domain: "Intrusion",
        options: [
          { label: "Not at all", value: 0 },
          { label: "A little bit", value: 1 },
      ...
      {
        id: "pcl-5",
        text: "Having strong physical reactions when reminded of the stressful experience",
        domain: "Intrusion",
        options: [
          { label: "Not at all", value: 0 },
          { label: "A little bit", value: 1 },
...
const BILLING_CODES = [
  ...
  {
    code: "H2019",
    description: "Therapeutic Behavioral Service",
    rate: "$110",
  },
...
const QUICK_ACTIONS = [
  ...
  },
  {
    ...
    icon: ClipboardCheck,
    description: "Open the synthetic, local-only measure preview",
  },
  {
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p
          ...
          style={{ color: alert ? "#DC2626" : "var(--topbar-title)" }}
        >
          {value}
        </p>
      </div>
...
function SoapNoteModal({
  ...
  const { data: patientsData } = trpc.bhc.listPatients.useQuery({
    pageSize: 100,
  });
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Document the clinical session using the SOAP format. All fields
        ...
        <div className="mb-2">
          ...
          <Select
            value={soapNote.billingCode}
            onValueChange={(v) =>
              setSoapNote((s) => ({ ...s, billingCode: v }))
            }
          >
            <SelectTrigger className="w-full">
          ...
          <div className="flex items-center gap-2">
            ...
            {lastSaved && !saving && (
              <span className="text-[11px] text-green-600 flex items-center gap-1">
                <CheckCircle2 size={11} /> Auto-saved at{" "}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="subjective" className="mt-3 space-y-2">
            <Label className="text-[12px]">
              Subjective — Patient&apos;s reported symptoms, concerns, and
              perspective
            </Label>
            <Textarea
            ...
            <div
              ...
              style={{ color: "var(--topbar-subtitle)" }}
            >
              Tip: Include chief complaint, HPI, and patient&apos;s own words in
              quotes.
            </div>
          </TabsContent>
          ...
          <TabsContent value="objective" className="mt-3 space-y-2">
            ...
            <div
              ...
              style={{ color: "var(--topbar-subtitle)" }}
            >
              Tip: Document MSE findings, observed behaviors, and objective
              measurements.
            </div>
          </TabsContent>
          ...
          <TabsContent value="assessment" className="mt-3 space-y-2">
            ...
            <div
              ...
              style={{ color: "var(--topbar-subtitle)" }}
            >
              Tip: Include diagnosis codes, risk level, and progress toward
              treatment goals.
            </div>
          </TabsContent>
          ...
          <TabsContent value="plan" className="mt-3 space-y-2">
            ...
            <div
              ...
              style={{ color: "var(--topbar-subtitle)" }}
            >
              Tip: Document interventions used, patient response, homework, and
              next session date.
            </div>
          </TabsContent>
        </Tabs>
...
// ═══════════════════════════════════════════════════════════════════════════════
//  D007-04: SYNTHETIC LEGACY MEASURE PREVIEW — NO RECORD WRITE
// ═══════════════════════════════════════════════════════════════════════════════
...
function OutcomeMeasureModal({
  ...
  if (!M41C_LEGACY_OUTCOME_MEASURE_PREVIEW_ENABLED) {
    return (
      <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
        <DialogContent
          className="max-w-xl"
          data-testid="m41c-legacy-outcome-measure-quarantine"
        >
          <DialogHeader>
            <DialogTitle>Governed Outcome Measure Experience</DialogTitle>
            <DialogDescription>
              The inherited PHQ-A and PCL-5 previews are quarantined because
              their item sets, versions, scoring, licensing, and validation are
              incomplete. This workspace cannot display those items, calculate
              totals or severity, save a result, route care, or update a patient
              record.
            </DialogDescription>
          </DialogHeader>
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-950">
            Use the Clinical Intelligence Fabric to evaluate governed profile
            metadata, source and version controls, qualification requirements,
            validation state, human review, and synthetic pathway behavior.
          </div>
          <DialogFooter>
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg bg-[#245C5A] px-4 py-2 text-sm font-semibold text-white"
            >
              Close
            </button>
          </DialogFooter>
  ...
  const severityLevel =
    measureDef?.severityLevels.find(
      (sl) => totalScore >= sl.min && totalScore <= sl.max,
    ) ?? null;
    ...
    >
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Synthetic legacy evaluation only. These incomplete previews are not
            validated for clinical use and cannot update a patient record.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  ...
                  backgroundColor:
                    answeredCount === totalQuestions ? "#059669" : "#7C3AED",
                }}
              />
            </div>
            ...
            {answeredCount > 0 && (
              <div
                ...
                style={{
                  borderColor: (severityLevel?.color ?? "#999") + "40",
                  backgroundColor: (severityLevel?.color ?? "#999") + "08",
                }}
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p
                      ...
                      style={{
                        color: severityLevel?.color ?? "var(--topbar-title)",
                      }}
                    >
                      {totalScore}
                      <span
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {Array.from(
                    new Set(measureDef.questions.map((q) => q.domain)),
                  ).map((domain) => {
                    const domainQuestions = measureDef.questions.filter(
                      (q) => q.domain === domain,
                    );
                    const domainScore = domainQuestions.reduce(
                      (s, q) => s + (responses[q.id] || 0),
                      0,
                    );
                    const domainMax = domainQuestions.reduce(
                      (s, q) => s + Math.max(...q.options.map((o) => o.value)),
                      0,
                    );
                    return (
            ...
            {submitted && (
              <div className="p-3 rounded-lg bg-green-50 border border-green-200 text-green-700 text-[13px] flex items-center gap-2">
                ...
                Synthetic preview completed locally. No patient record was
                written.
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-3 mt-2">
          ...
          <div>
            ...
            <Select
              value={form.gender}
              onValueChange={(v) =>
                setForm((f) => ({ ...f, gender: v as typeof form.gender }))
              }
            >
              <SelectTrigger className="w-full">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, insuranceId: e.target.value }))
              }
              className="text-[13px]"
            />
          </div>
          <div>
            ...
            <Input
              ...
              className="text-[13px]"
              placeholder="e.g. HISD School Counselor"
            />
          </div>
          <div>
          ...
          <div className="col-span-2">
            ...
            <Input
              value={form.assignedClinicianId}
              onChange={(e) =>
                setForm((f) => ({ ...f, assignedClinicianId: e.target.value }))
              }
              className="text-[13px]"
        ...
        {!started ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                {["individual", "group", "family"].map((type) => (
                  <button
                    ...
                    style={{
                      backgroundColor:
                        sessionType === type ? "#2563EB" : "var(--card-bg)",
                      color:
                        sessionType === type
                          ? "#fff"
                          : "var(--topbar-subtitle)",
                      borderColor:
                        sessionType === type ? "#2563EB" : "var(--card-border)",
                    }}
                  >
                    {type}
                  </button>
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{
                      backgroundColor:
                        STATUS_COLORS[selectedPatient.status] ?? "#6B7280",
                    }}
                  >
          ...
          <div className="py-12 text-center">
            ...
            <p
              ...
              style={{ color: "var(--topbar-subtitle)" }}
            >
              Timer running for {selectedPatient?.firstName}{" "}
...
function ViewTreatmentPlanModal({
  open,
  onClose,
}: {
  open: boolean;
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Quick access to a patient&apos;s active treatment plan.
          </DialogDescription>
        </DialogHeader>
...
export function ClinicalWorkspacePage() {
  ...
  const { data: patientsData, isLoading: patientsLoading } =
    trpc.bhc.listPatients.useQuery({ pageSize: 100 });
  const { data: sessionsData } = trpc.bhc.listSessions.useQuery({
    status: "scheduled",
  });
  trpc.bhc.clinicianWorkload.useQuery();
  ...
  const [directoryFilter, setDirectoryFilter] = useState<
    "all" | "active" | "today" | "pending-docs"
  >("all");

  // ─── Derived Data ──────────────────────────────────────────────────────────
  const allPatients = patientsData?.patients ?? [];
  const activePatients = allPatients.filter((p) => p.status === "active");
  const censusCount = allPatients.filter(
    (p) => p.status === "active" || p.status === "hold",
  ).length;
  ...
  const overdueAssessmentIds = ["p4"]; // Synthetic Youth 002 on hold
  ...
  const filteredPatients = allPatients.filter((patient) => {
    ...
    if (directoryFilter === "today") {
      matchesFilter = (sessionsData ?? []).some(
  ...
  const renderModal = () => {
    switch (activeModal) {
      case "admit":
        return (
          <AdmitPatientModal open={true} onClose={() => setActiveModal(null)} />
        );
      case "session":
        return (
          <StartSessionModal open={true} onClose={() => setActiveModal(null)} />
        );
      case "soap":
        return (
          <SoapNoteModal open={true} onClose={() => setActiveModal(null)} />
        );
      case "outcome":
        return (
          <OutcomeMeasureModal
  ...
  const quickStats = [
    ...
    },
    {
      ...
      value: dashboardKPIs?.sessionsToday ?? 0,
      icon: Calendar,
      color: "#D97706",
    },
    {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              },
              {
                ...
                icon: ArrowRightLeft,
                color: "#D97706",
                desc: "Cross-dept referrals",
              },
              {
                label: "CANS Governance",
                href: "/clinical/cans-assessments",
                icon: ClipboardCheck,
                color: "#EA580C",
                desc: "Quarantined legacy scoring and governed replacement",
              },
              {
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {(
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor:
                        directoryFilter === f.key ? "#245C5A" : "transparent",
                      color:
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              <thead>
                ...
                >
                  <th
                    className="text-left px-4 py-3 font-semibold text-[12px]"
                    style={{ color: "var(--topbar-title)" }}
                  >
                    Patient
                  </th>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div
                            ...
                            style={{
                              backgroundColor:
                                STATUS_COLORS[patient.status] ?? "#6B7280",
                            }}
                          >
                            {patient.firstName[0]}
                            {patient.lastName[0]}
              ...
              {sessionsData.map((session) => {
                ...
                return (
                  <div
                    ...
                    style={{ borderColor: "var(--card-border)" }}
                    onClick={() =>
                      navigate(`/clinical/patients/${session.patientId}`)
                    }
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        ...
                        &bull; {session.sessionType} &bull;{" "}
                        {session.durationMinutes}min &bull;{" "}
                        {session.billingCode ?? "No code"}
                      </p>
                    </div>
                    <ChevronRight
                      size={14}
