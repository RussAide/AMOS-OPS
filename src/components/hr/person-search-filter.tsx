...
// Helper: filter people based on FilterState
export function useFilteredPeople(filters: FilterState) {
  ...
  return useMemo(() => {
    return people.filter((p) => {
      ...
      const matchesLane = filters.lane === "all" || p.lane === filters.lane;
      const matchesDept = filters.department === "all" || p.department === filters.department;
      const matchesSuper = filters.supervisor === "all" || p.supervisor === filters.supervisor;
      const matchesModule = !filters.moduleId || !filters.statusId ||
        p.moduleStatuses[filters.moduleId] === filters.statusId;
      return matchesSearch && matchesLane && matchesDept && matchesSuper && matchesModule;
    });
  }, [people, filters]);
}
