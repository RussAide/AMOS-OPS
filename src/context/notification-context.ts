...
export function NotificationProvider({ children }: { children: ReactNode }) {
  const [localNotifications, setLocalNotifications] = useState<AppNotification[]>(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) return parsed as AppNotification[]
      }
    } catch {
  ...
  const notifications: AppNotification[] = apiNotifications && apiNotifications.length > 0
