...
export function HRProvider({ children }: { children: ReactNode }) {
  ...
  const [localPeople, setLocalPeople] = useState<HRPerson[]>([
    { id: "p-001", firstName: "Marcus", lastName: "Johnson", role: "GRO Specialist", department: "Field Operations", lane: "activation", isActive: true, isEmployee: true, employeeId: "E-1045", hireDate: "2024-01-15", supervisor: "Robert Fitzgerald", moduleStatuses: { recruitment: "r-closed", screening: "s-selected", interview: "i-completed", offers: "o-file-ready", orientation: "or-signed", onboarding: "ob-comp-done", clearance: "c-cleared", "personnel-files": "pf-complete", credentials: "cr-current", performance: "pa-closed", compliance: "ca-closed" } },
  ...
  const [localTransitions, setLocalTransitions] = useState<StatusTransition[]>([
    { id: "t-001", personId: "p-001", personName: "Marcus Johnson", moduleId: "recruitment", moduleName: "Recruitment", fromStatus: "(none)", toStatus: "Closed", changedBy: "HR Director", changedAt: "2024-01-10T10:00:00Z" },
    { id: "t-002", personId: "p-001", personName: "Marcus Johnson", moduleId: "clearance", moduleName: "Clearance", fromStatus: "Pending", toStatus: "Cleared", changedBy: "QA Officer", changedAt: "2024-02-20T14:00:00Z" },
  ...
  useEffect(() => {
    if (rawPeople.length > 0) {
      const merged = mergeModuleStatuses(
        rawPeople as unknown as RawPerson[],
        rawStatuses as unknown as RawStatus[]
      );
  ...
  useEffect(() => {
    if (rawTransitions.length > 0) {
      setLocalTransitions(rawTransitions.map((t: Record<string, unknown>) => ({
        id: String(t.id),
        personId: String(t.personId),
        personName: String(t.personName),
        moduleId: String(t.moduleId),
        moduleName: String(t.moduleName),
        fromStatus: String(t.fromStatus),
        toStatus: String(t.toStatus),
        changedBy: String(t.changedBy),
        changedAt: String(t.changedAt || new Date().toISOString()),
  ...
  useEffect(() => {
    if (rawDocuments.length > 0) {
      setLocalDocuments(rawDocuments.map((d: Record<string, unknown>) => ({
        id: String(d.id),
        personId: String(d.personId),
        moduleId: String(d.moduleId),
        recordName: String(d.recordName),
        fileName: String(d.fileName),
        fileType: String(d.fileType || ""),
        fileSize: Number(d.fileSize || 0),
        uploadedAt: String(d.uploadedAt || new Date().toISOString()),
        uploadedBy: d.uploadedBy ? String(d.uploadedBy) : "",
        verifiedAt: d.verifiedAt ? String(d.verifiedAt) : undefined,
        verifiedBy: d.verifiedBy ? String(d.verifiedBy) : undefined,
        status: (d.status || "uploaded") as "uploaded" | "verified" | "rejected" | "expired",
  ...
  const updatePersonStatus = useCallback(
    async (personId: string, moduleId: string, statusId: string) => {
      ...
      try {
        ...
        if (transitionResult) {
          const tr = transitionResult as Record<string, unknown>;
          setLocalTransitions((prev) => [{
            id: String(tr.id),
            personId: String(tr.personId),
            personName: String(tr.personName),
            moduleId: String(tr.moduleId),
            moduleName: String(tr.moduleName),
            fromStatus: String(tr.fromStatus),
            toStatus: String(tr.toStatus),
            changedBy: String(tr.changedBy),
            changedAt: String(tr.changedAt || new Date().toISOString()),
            note: tr.note ? String(tr.note) : undefined,
          }, ...prev]);
        }
