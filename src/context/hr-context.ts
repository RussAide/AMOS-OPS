...
export function mayUseHrDemoData(
...
export function resolveHrCollection<T>(
...
export function mergeModuleStatuses(
  rawPeople: RawPerson[],
  statuses: RawStatus[],
...
interface HRState {
  ...
  // Actions
  createPerson: (
    person: Omit<HRPerson, "id" | "moduleStatuses" | "isActive">,
  ) => HRPerson;
  updatePerson: (
    personId: string,
    updates: Partial<Omit<HRPerson, "id" | "moduleStatuses">>,
  ) => void;
  updatePersonStatus: (
    personId: string,
    moduleId: string,
  ...
  getDocumentsForPersonAndModule: (
    personId: string,
    moduleId: string,
  ) => HRDocument[];
  ...
  getMissingDocumentsGlobally: () => Array<{
    personId: string;
    personName: string;
    moduleId: string;
    moduleName: string;
...
export function HRProvider({ children }: { children: ReactNode }) {
  ...
  const demoAllowed = mayUseHrDemoData(runtimeConfig.evaluationMode, workspace);

  // ─── tRPC Queries ──────────────────────────────────────────
  const peopleQuery = trpc.hr.listPeople.useQuery();
  const statusesQuery = trpc.hr.getModuleStatuses.useQuery(undefined);
  const transitionsQuery = trpc.hr.listTransitions.useQuery({});
  const documentsQuery = trpc.documents.list.useQuery({});

  // Demo fixtures are allowed only in evaluation/training. Production starts empty.
      ...
      ? [
          ...
          },
          {
            ...
            moduleStatuses: {
              ...
              interview: "i-completed",
              offers: "o-file-ready",
              orientation: "or-in-progress",
              onboarding: "ob-in-progress",
              clearance: "c-pending-bg",
              "personnel-files": "pf-incomplete",
              credentials: "cr-current",
              performance: "pa-closed",
              compliance: "ca-closed",
            },
          },
          {
            ...
            supervisor: "Robert Fitzgerald",
            moduleStatuses: {
              recruitment: "r-posted",
              screening: "s-interview-sched",
              interview: "i-not-started",
              offers: "o-not-started",
              orientation: "or-not-started",
              onboarding: "ob-not-started",
              clearance: "c-not-ready",
              "personnel-files": "pf-not-started",
              credentials: "cr-current",
              performance: "pa-closed",
              compliance: "ca-closed",
            },
          ...
          },
          {
            ...
            moduleStatuses: {
              ...
              offers: "o-file-ready",
              orientation: "or-signed",
              onboarding: "ob-cert-pending",
              clearance: "c-super-review",
              "personnel-files": "pf-complete",
              credentials: "cr-expiring",
              performance: "pa-closed",
              compliance: "ca-closed",
            },
          },
    ...
    demoAllowed
      ? [
          ...
          },
          {
            ...
            fromStatus: "Pending",
            toStatus: "Cleared",
            changedBy: "QA Officer",
            changedAt: "2024-02-20T14:00:00Z",
          },
          {
            id: "t-003",
            personId: "p-009",
      ...
      ? [
          {
            ...
            verifiedAt: "2024-01-11T14:00:00Z",
          ...
          },
          {
            ...
            personId: "p-001",
            moduleId: "screening",
            recordName: "Background Check",
            fileName: "bg_check_marcus.pdf",
            fileType: "application/pdf",
            fileSize: 512000,
          ...
          },
          {
            ...
            personId: "p-009",
            moduleId: "performance",
            recordName: "Q2 Review",
            fileName: "q2_review_robert.pdf",
            fileType: "application/pdf",
            fileSize: 1200000,
            uploadedAt: "2024-05-01T10:00:00Z",
            uploadedBy: "HR Director",
            verifiedAt: "2024-05-03T14:00:00Z",
            verifiedBy: "HR Director",
            status: "verified",
          },
        ]
  ...
  useEffect(() => {
    if (!peopleQuery.isFetched || !statusesQuery.isFetched) return;
    const authoritativePeople =
      peopleQuery.isError || statusesQuery.isError
        ? []
        : mergeModuleStatuses(
            peopleQuery.data ?? [],
            statusesQuery.data ?? [],
    ...
    queueMicrotask(() =>
      setLocalPeople((demo) =>
        resolveHrCollection(authoritativePeople, demo, demoAllowed),
      ),
    );
  }, [
    demoAllowed,
    peopleQuery.data,
    peopleQuery.isError,
    peopleQuery.isFetched,
    statusesQuery.data,
    statusesQuery.isError,
    statusesQuery.isFetched,
  ]);

  useEffect(() => {
    if (!transitionsQuery.isFetched) return;
    const transitions: StatusTransition[] = transitionsQuery.isError
      ? []
    ...
    queueMicrotask(() =>
      setLocalTransitions((demo) =>
        resolveHrCollection(transitions, demo, demoAllowed),
      ),
    );
  }, [
    demoAllowed,
  ...
  useEffect(() => {
    if (!documentsQuery.isFetched) return;
    const documents: HRDocument[] = documentsQuery.isError
      ? []
    ...
    queueMicrotask(() =>
      setLocalDocuments((demo) =>
        resolveHrCollection(documents, demo, demoAllowed),
      ),
    );
  }, [
    demoAllowed,
    ...
    documentsQuery.isFetched,
  ]);

  const isLoading =
    (peopleQuery.isLoading || statusesQuery.isLoading) &&
      ...
      setLocalPeople((prev) =>
        prev.map((p) =>
          p.id === personId
            ? {
                ...p,
                moduleStatuses: { ...p.moduleStatuses, [moduleId]: statusId },
              }
            : p,
        ),
      );
  ...
  const _getDocumentsForPersonAndModule = useCallback(
    (personId: string, moduleId: string) =>
      localDocuments.filter(
        (d) => d.personId === personId && d.moduleId === moduleId,
      ),
    [localDocuments],
  );
  ...
  const _getMissingDocumentsGlobally = useCallback(() => {
    ...
    for (const person of localPeople) {
      for (const mod of ALL_HR_MODULES) {
        ...
        const uploaded = docs.map((d) => d.recordName);
        const missing = mod.requiredRecords.filter(
          (r) => !uploaded.includes(r),
        );
        if (missing.length > 0) {
          results.push({
            personId: person.id,
  ...
  const getPeopleInModule = useCallback(
    (moduleId: string) =>
      localPeople.filter((p) => moduleId in p.moduleStatuses),
    [localPeople],
  );
  ...
  const getActiveAlerts = useCallback(() => {
    return ALERT_RULES.map((rule) => {
      ...
      if (rule.moduleId === "clearance" && rule.id === "al-1") {
        ...
        affected = localPeople.filter(
          (p) => p.moduleStatuses.credentials === "cr-expired",
        );
      } else if (rule.moduleId === "orientation") {
        affected = localPeople.filter(
          (p) => p.moduleStatuses.orientation === "or-in-progress",
        );
      } else if (rule.moduleId === "personnel-files") {
        affected = localPeople.filter(
          (p) => p.moduleStatuses["personnel-files"] === "pf-incomplete",
        );
      } else if (rule.moduleId === "clearance" && rule.id === "al-5") {
        affected = localPeople.filter(
          (p) => p.moduleStatuses.clearance === "c-restricted",
        );
      }
  ...
  const getCardMetrics = useCallback(() => {
    return {
      active_requisitions: localPeople.filter(
        (p) => p.moduleStatuses.recruitment === "r-posted",
      ).length,
      pending_packets: localPeople.filter((p) =>
        ["o-sent", "o-packet-sent", "o-packet-inc", "o-file-build"].includes(
          p.moduleStatuses.offers,
        ),
      ).length,
      ...
      incomplete_training: localPeople.filter((p) =>
        [
          ...
          "ob-cert-pending",
          "ob-comp-pending",
        ].includes(p.moduleStatuses.onboarding),
      ).length,
      pending_reviews: localPeople.filter((p) =>
        [
          ...
          "c-hr-review",
          "c-super-review",
        ].includes(p.moduleStatuses.clearance),
      ).length,
      incomplete_files: localPeople.filter((p) =>
        ["pf-incomplete", "pf-missing"].includes(
          p.moduleStatuses["personnel-files"],
        ),
      ).length,
      expiring_soon: localPeople.filter(
        (p) => p.moduleStatuses.credentials === "cr-expiring",
      ).length,
      open_actions: localPeople.filter((p) =>
        ["pa-open", "pa-notified", "pa-followup"].includes(
          p.moduleStatuses.performance,
        ),
      ).length,
    };
  }, [localPeople]);
